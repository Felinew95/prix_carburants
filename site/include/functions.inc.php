    ...
    function getDepartements(Region $region) : array {
        ...
        while ($ligneDepartement = fgets($fichierDepartements)) {
            ...
            if ($codeRegionCsv === $codeRegion) {
                $departements[$codeDepartement] = new Departement($nomDepartement, $codeDepartement, $region);
            }
        }
    ...
    function getCommunes(Departement $departement) : array {
        ...
        while ($ligneCommune = fgets($fichierCommunes)) {
            ...
            if (substr($codePostal, 0, 2) === $codeDepartement) {
                $communes[$codeCommuneInsee] = new Ville($nomCommune, $codeCommuneInsee, $departement, $codePostal, $latitude, $longitude);   
            }
        }
