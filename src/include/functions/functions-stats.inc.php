<?php
    ...
    function getNombreTotalStationsFrance(int $tempsCache = TEMPS_CACHE_NOMBRE_STATIONS_FRANCE): int {
        creerDossier(dirname(FICHIER_NOMBRE_STATIONS));
        creerFichier(FICHIER_NOMBRE_STATIONS);

        $donneesCache = file_get_contents(FICHIER_NOMBRE_STATIONS);
        if (file_exists(FICHIER_NOMBRE_STATIONS) && (time() - filemtime(FICHIER_NOMBRE_STATIONS)) < $tempsCache && !empty($donneesCache)) {
            return (int) $donneesCache;
        }
        ...
        if (!is_array($donnees) || !isset($donnees['total_count'])) {
            return 0;
        }
    
        $nombreStations = (int) $donnees['total_count'];
        file_put_contents(FICHIER_NOMBRE_STATIONS, (string) $nombreStations);
